                    ...
                    if (!keywordList.Contains(finalArrayc[x].ToString())) // if it is not a keyword
                     {
                           ...
                           Regex reg1 = new Regex(@"^(int|String|float|double)\s([A-Za-z|_][A-Za-z|0-z]{0,10})\s(=)\s([0-9]+([.][0-9]+)?([e][+|-]?[0-9]+)?)\s(;)$"); // line of type int alpha = 2;
                           ...
                           Match category2 = reg2.Match(line); //if it is a category 1 then add a row in symbol table containing the information related to that variable
